...
public class Biker {
    ...
    public double getBestGain() {
    	...
    	double bestgain=0;
    	ArrayList<Double> gains = new ArrayList<Double>(averageSpeeds.values());
    	for(int i=1;i<gains.size();i++){
    		if(gains.get(i)-gains.get(i-1)>bestgain){
    			bestgain=gains.get(i)-gains.get(i-1);
    		}
    	}
    	return bestgain;
    ...
    public double getMedianSpeed() {
    	...
    	ArrayList <Double> speeds = new ArrayList(averageSpeeds.values());
    	
    	for (int i=1;i<averageSpeeds.size();i++){
    		if(speeds.get(i)<speeds.get(i-1)){
    			temp=speeds.get(i);
    			speeds.add(i, speeds.get(i-1));
    			speeds.set(i-1, temp);
    			i=1;
    		}
    	}
        ...
        if(speeds.size()%2==0){
        	return speeds.get(((speeds.size()/2)+((speeds.size()/2)+1))/2);
        }
        else{
